package Panels
{
   ...
   public class Encyclopedia_Npc extends MovieClip
   {
      ...
      private function loadPreviewSwf(param1:MouseEvent) : void
      {
         ...
         var _loc4_:*;
         (_loc4_ = this.loaderSwf.add(_loc3_)).addEventListener(BulkLoader.COMPLETE,this.onCompleteNpcLoaded);
      ...
      private function searchItem(param1:MouseEvent) : void
      {
         ...
         var _loc4_:Array;
         var _loc5_:int = (_loc4_ = this.npcDataOriginal).length;
         var _loc7_:int = 0;
         while(_loc7_ < _loc5_)
         {
            _loc6_ = _loc4_[_loc7_];
            if((_loc9_ = (_loc8_ = NpcInfo.getNpcStats(_loc6_))["npc_name"].toLowerCase()).indexOf(_loc3_) >= 0)
            {
               _loc2_.push(_loc6_);
            }
            _loc7_++;
