package com.abrahamyan.liquid
{
   ...
   public class ToolTip extends MovieClip
   {
      ...
      public function show(param1:String = "", param2:Number = 0, param3:Number = 0) : void
      {
         ...
         if(this._multiLine)
         {
            if((_loc4_ = this._label.replace(/<[^>]*>/g,"")).length <= 15)
            {
               this.label_txt.wordWrap = false;
               this.label_txt.autoSize = TextFieldAutoSize.LEFT;
            }
      ...
      {
         var _loc1_:Number = !!this._followMouse ? Number(Number(Number(Number(stage.mouseX + this._xposition)))) : Number(Number(Number(Number(this._xposition))));
         var _loc2_:Number = !!this._followMouse ? Number(Number(Number(Number(stage.mouseY + this._yposition)))) : Number(Number(Number(Number(this._yposition))));
         ...
         this.x = Math.round(this._horDirection == DIRECTION_LEFT ? Number(Number(Number(Number(_loc1_ - this.label_txt.width)))) : Number(Number(Number(Number(_loc1_)))));
         this.y = Math.round(this._verticalDirection == DIRECTION_TOP ? Number(Number(Number(Number(_loc2_ - this.label_txt.height)))) : Number(Number(Number(Number(_loc2_)))));
      }
