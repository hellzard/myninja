package com.gestouch.core
{
   ...
   public class TouchesManager
   {
      ...
      gestouch_internal function onTouchBegin(param1:uint, param2:Number, param3:Number, param4:Object = null) : Boolean
      {
         ...
         (_loc7_ = this.createTouch()).id = param1;
         for each(_loc10_ in this._hitTesters)
         {
            if(_loc8_ = _loc10_.hitTest(_loc5_,param4))
            {
               if(!(_loc8_ is Stage))
               {
                  break;
         ...
         _loc7_.setLocation(param2,param3,getTimer());
         this._touchesMap[param1] = _loc7_;
         ++this._activeTouchesCount;
         this._gesturesManager.onTouchBegin(_loc7_);
         return true;
      }
      ...
      {
         var _loc4_:Touch;
         if(!(_loc4_ = this._touchesMap[param1] as Touch))
         {
            return;
         }
         if(_loc4_.updateLocation(param2,param3,getTimer()))
         {
            this._gesturesManager.onTouchMove(_loc4_);
         }
      }
      ...
      gestouch_internal function onTouchEnd(param1:uint, param2:Number, param3:Number) : void
      {
         ...
         --this._activeTouchesCount;
         this._gesturesManager.onTouchEnd(_loc4_);
         _loc4_.target = null;
      }
      ...
      gestouch_internal function onTouchCancel(param1:uint, param2:Number, param3:Number) : void
      {
         ...
         delete this._touchesMap[param1];
         --this._activeTouchesCount;
         this._gesturesManager.onTouchCancel(_loc4_);
         _loc4_.target = null;
      }
      ...
      protected function hitTestersSorter(param1:ITouchHitTester, param2:ITouchHitTester) : Number
      {
         ...
         return this._hitTesters.indexOf(param1) > this._hitTesters.indexOf(param2) ? Number(1) : Number(-1);
      }
   }
}
