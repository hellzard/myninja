package fl.controls
{
   ...
   public class LabelButton extends BaseButton implements IFocusManagerComponent
   {
      ...
      {
         return !!this._toggle ? Boolean(Boolean(_selected)) : false;
      }
      ...
      override protected function drawLayout() : void
      {
         ...
         var _loc7_:Number = this.icon == null ? Number(Number(0)) : Number(Number(this.icon.width + _loc3_));
         var _loc8_:Number = this.icon == null ? Number(Number(0)) : Number(Number(this.icon.height + _loc3_));
         ...
         else if(_loc4_ == ButtonLabelPlacement.BOTTOM || _loc4_ == ButtonLabelPlacement.TOP)
         {
            ...
            if(this.icon != null)
            {
               this.icon.y = Math.round(_loc4_ == ButtonLabelPlacement.BOTTOM ? Number(Number(this.textField.y - _loc8_)) : Number(Number(this.textField.y + this.textField.height + _loc3_)));
            }
         }
         else
         {
            _loc1_ = Math.max(0,Math.min(_loc5_,width - _loc7_ - 2 * _loc3_));
            this.textField.x = Math.round((width - (this.textField.width = Number(_loc1_)) - _loc7_) / 2 + (_loc4_ != ButtonLabelPlacement.LEFT ? _loc7_ : 0));
            this.textField.y = Math.round((height - this.textField.height) / 2);
            if(this.icon != null)
            {
               this.icon.x = Math.round(_loc4_ != ButtonLabelPlacement.LEFT ? Number(Number(this.textField.x - _loc7_)) : Number(Number(this.textField.x + _loc5_ + _loc3_)));
            }
         }
         super.drawLayout();
      }
