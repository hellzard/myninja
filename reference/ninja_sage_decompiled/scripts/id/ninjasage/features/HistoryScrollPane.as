package id.ninjasage.features
{
   ...
   public class HistoryScrollPane extends MovieClip
   {
      ...
      public function updatePane(param1:Object) : void
      {
         ...
         var _loc2_:Number = !!this.paneData.hasOwnProperty("text_width") ? Number(Number(this.paneData.text_width)) : Number(280);
         var _loc3_:Number = !!this.paneData.hasOwnProperty("x") ? Number(Number(this.paneData.x)) : Number(0);
         var _loc4_:Number = !!this.paneData.hasOwnProperty("y") ? Number(Number(this.paneData.y)) : Number(0);
         ...
         {
            if((_loc9_ = _loc6_[_loc8_] as Object) != null)
            {
               _loc10_ = !!_loc9_.hasOwnProperty("text") ? String(_loc9_.text) : "";
               _loc12_ = !!_loc9_.hasOwnProperty("marginAfter") ? Number(Number(_loc9_.marginAfter)) : Number(4);
               if(!_loc10_.length)
               {
               ...
               else
               {
                  ...
                  (_loc13_ = new TextField()).type = TextFieldType.DYNAMIC;
         ...
         var _loc14_:Number = this.paneData.hasOwnProperty("width") && this.paneData.width != null ? Number(Number(this.paneData.width)) : Number(Number(_loc3_ + _loc2_ + 20));
         var _loc15_:Number = this.paneData.hasOwnProperty("height") && this.paneData.height != null ? Number(Number(this.paneData.height)) : Number(400);
      ...
      {
         return param1.color != null ? uint(uint(param1.color)) : uint(16777215);
      }
      ...
      private function buildLinesFromHistoryRaw(param1:String, param2:TextFormat) : Array
      {
         ...
         {
            if((_loc7_ = String(_loc5_[_loc6_]).replace(/^\s+|\s+$/g,"")).length)
            {
               if(_loc8_ = _loc7_.match(_loc9_))
               {
                  _loc3_.push({
                     "text":"[" + _loc8_[1] + "]",
                     "format":param2,
      ...
      private function textFormatForHistoryDescription(param1:String, param2:TextFormat) : TextFormat
      {
         ...
         var _loc4_:Array;
         if(_loc4_ = param1.match(/\bdealt\s+([+-]?\d+)\s+damage/i))
         {
