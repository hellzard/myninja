package Combat
{
   ...
   public class AgilityBarManager
   {
      ...
      public var team_headMc_agility:Vector.<AgilityBarEntry> = new Vector.<AgilityBarEntry>();
      
      public var plus_x_next_round:Vector.<Number> = new Vector.<Number>();
      ...
      public function setupHeadsAndAgility() : *
      {
         ...
         for(_loc3_ in this.players_mc_holders)
         {
            if("character_model" in this.players_mc_holders[_loc3_].charMc)
            {
               ...
               _loc2_.x = 0;
               _loc2_.y = _loc7_;
               _loc2_.scaleX = _loc1_.isCharacter() ? 0.5 : -0.5;
               _loc2_.scaleY = _loc1_.isCharacter() ? 0.5 : 0.5;
               _loc5_ = this.getCalculatedAgility(_loc1_);
               if(_loc5_ > _loc6_)
               {
                  _loc6_ = _loc5_;
               }
      ...
      public function startRun(param1:int = 0) : *
      {
         ...
         if(Boolean(this.destroyed) || _loc2_ == null)
         {
            return;
         }
         if("isDebugEnemyManualWaiting" in _loc2_ && Boolean(_loc2_.isDebugEnemyManualWaiting()))
         {
            return;
         }
         if(param1 > 0)
      ...
      private function handleFirstTurnInit(param1:*) : Boolean
      {
         ...
         _loc2_ = new Animation(param1,false);
      ...
      private function handleBattleInterrupts() : Boolean
      {
         ...
         var _loc2_:Boolean = !_loc1_ ? this.checkReviveEOM() : false;
      ...
      private function isPlayerTeamDefeated(param1:Battle) : Boolean
      {
         ...
         {
            if(Boolean(param1.character_team_players[_loc3_].health_manager.isDead()) && (_loc2_ || _loc3_ == 0))
            {
               return true;
            }
            _loc3_++;
      ...
      private function isEnemyTeamDefeated(param1:Battle) : Boolean
      {
         ...
         return this.isSpecialJouninStage() && Boolean(param1.enemy_team_players[0].health_manager.isDead());
      }
      ...
      {
         return Boolean(Character.is_jounin_stage_4) || Boolean(Character.is_jounin_stage_5_1) || Boolean(Character.is_jounin_stage_5_2);
      }
      ...
      private function hasTeamUnyieldingActivation(param1:*) : Boolean
      {
         ...
         {
            _loc2_ = param1[_loc3_];
            if(Boolean(_loc2_.isCharacter()) && Boolean(_loc2_.health_manager.checkActivateUnyielding()))
            {
               return true;
            }
            _loc3_++;
      ...
      public function checkAmbush() : void
      {
         ...
         _loc4_ = this.applyAmbushMovement(_loc3_,_loc1_,_loc2_);
         if(_loc4_ >= 0)
         {
      ...
      private function prepareAmbushState(param1:Vector.<Number>, param2:Vector.<Number>) : Number
      {
         ...
         while(_loc7_ < this.team_headMc_agility.length)
         {
            ...
            else
            {
               ...
               _loc6_ = this.getAmbushStepsNeeded(_loc4_,param2[_loc7_]);
               if(_loc6_ < _loc5_)
               {
                  _loc5_ = _loc6_;
               }
      ...
      private function getAmbushStepsNeeded(param1:Number, param2:Number) : Number
      {
         ...
         return _loc3_ <= 0 ? 1 : Math.ceil(_loc3_ / param2);
      }
      ...
      private function applyAmbushMovement(param1:Number, param2:Vector.<Number>, param3:Vector.<Number>) : int
      {
         ...
         {
            _loc4_ = this.team_headMc_agility[_loc8_];
            if(_loc4_.model.isDead())
            {
               this.clearAmbushTransientStateForIndex(_loc8_,_loc4_);
            }
            else
            {
               _loc5_ = param2[_loc8_] + param1 * param3[_loc8_];
               if(_loc5_ >= this.action_bar_width)
               {
                  if(_loc5_ > _loc6_)
                  {
                     _loc6_ = _loc5_;
      ...
      public function updateAgilityToActionBar() : *
      {
         ...
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = int(this.team_headMc_agility.length);
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc6_ = this.team_headMc_agility[_loc5_];
            _loc1_ = _loc6_.model;
...
class AgilityBarTeamInfo
{
   ...
   public function AgilityBarTeamInfo(param1:String, param2:int, param3:Boolean)
   {
      super();
