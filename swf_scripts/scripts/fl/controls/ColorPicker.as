package fl.controls
{
   ...
   [Embed(source="/_assets/assets.swf", symbol="symbol4454")]
   public class ColorPicker extends UIComponent implements IFocusManagerComponent
   {
      ...
      {
         if(Boolean(this.swatchSelectedSkin) && this.palette.contains(this.swatchSelectedSkin))
         {
      ...
      protected function drawSwatches() : void
      {
         ...
         var _loc3_:Number = this.showTextField ? Number(this.textFieldBG.y + this.textFieldBG.height + _loc2_) : Number(_loc2_);
         ...
         {
            _loc1_ = this.createSwatch(this.colors[_loc10_]);
            _loc1_.x = (_loc6_ + _loc5_) * (_loc10_ % _loc4_);
            if(_loc1_.x == 0)
            {
      ...
      protected function drawBG() : void
      {
         ...
         this.paletteBG.width = Math.max(this.showTextField ? Number(this.textFieldBG.width) : Number(0),this.swatches.width) + _loc2_ * 2;
      ...
      override protected function keyUpHandler(param1:KeyboardEvent) : void
      {
         ...
         {
            _loc3_ = this.textField.text;
            if(_loc3_.indexOf("#") > -1)
            {
               _loc3_ = _loc3_.replace(/^\s+|\s+$/g,"");
         ...
         {
            _loc2_ = uint(this.rollOverColor);
            _loc5_.color = _loc2_;
         }
      ...
      protected function setColorDisplay() : void
      {
         ...
         var _loc1_:ColorTransform = new ColorTransform(0,0,0,1,this._selectedColor >> 16,this._selectedColor >> 8 & 0xFF,this._selectedColor & 0xFF,0);
         ...
         if(Boolean(this.swatchMap.length) && this.colorHash[this._selectedColor] == undefined)
         {
            this.cleanUpSelected();
         }
      }
