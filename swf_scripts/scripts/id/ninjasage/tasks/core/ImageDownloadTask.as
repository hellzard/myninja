package id.ninjasage.tasks.core
{
   ...
   public class ImageDownloadTask extends EventDispatcher implements ITask
   {
      ...
      {
         if(Boolean(param1) && Boolean(param1.hasOwnProperty("status")) && param1.status == 1)
         {
      ...
      private function checkAndDownloadImages() : void
      {
         ...
         if(Boolean(this.data.hasOwnProperty("seasonal")) && Boolean(this.data.seasonal is Array) && this.data.seasonal.length > 0)
         {
            this.loaded = 0;
            this.downloading = 0;
            this.downloadQueue = [];
            ...
            while(_loc2_ < this.data.seasonal.length)
            {
               ...
               _loc5_ = "tmp/" + _loc4_;
               _loc6_ = File.applicationStorageDirectory.resolvePath(_loc5_);
               if(!_loc6_.exists)
               {
      ...
      private function loadImages() : void
      {
         ...
         if(Boolean(this.data.hasOwnProperty("seasonal")) && Boolean(this.data.seasonal is Array) && this.data.seasonal.length > 0)
         {
            ...
            while(_loc1_ < this.data.seasonal.length)
            {
               ...
               _loc7_ = "tmp/" + _loc6_;
               _loc8_ = File.applicationStorageDirectory.resolvePath(_loc7_);
               if(_loc8_.exists)
               {
                  _loc3_ = _loc8_.url;
               }
      ...
      internal function completeImg(param1:*) : *
      {
      ...
      internal function errImg(param1:*) : *
      {
         param1.currentTarget.removeEventListener(param1.type,arguments.callee);
         this.checkComplete();
      }
      
      internal function checkComplete() : *
      {
         ++this.loaded;
      ...
      internal function onImageComplete(param1:*) : void
      {
         this.complete();
      }
